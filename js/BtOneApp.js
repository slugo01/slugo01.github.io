...
class BtOneApp extends BLEDevice {
  ...
  async start() {
    await this.connect();
    const payload = new Int8Array([255, 3, 0, 12, 0, 2, 11, 214]);
    this.write(payload );
    const a= new Int8Array([255, 3, 1, 0, 0, 7, 10, 42]);
    this.timer =  window.setInterval( () => this.write(a), 10000);

  }
/*
  async datos() {
  payload = new Int8Array([255, 3, 1, 0, 0, 7, 10, 42]);
  this.write(payload);
  this.timer =  window.setInterval( () => this.write(payload), 10000);
  }
*/
  ...
  async toggleLoad() {
    if (this.parsedData && this.parsedData['load_status'] == 'off') {
      ...
      const payload = new Int8Array([255, 3, 1, 0, 0, 7, 10, 42]);
      this.write(payload);
    } else {
  ...
  onConnect() {  
   // const payload = new Int8Array([109, 61, 69, 110, 20, 72, 65,63,76,20,64,61,74,61,91,66,66,91,20,91]);
   // this.write(payload);
   // this.timer =  window.setInterval( () => this.write(payload), 10000);
    // const a= new Int8Array([255, 3, 1, 0, 0, 7, 10, 42]);
   // this.write(a);
   // this.timer =  window.setInterval( () => this.write(a), 20000);
    super.onConnect();
  ...
  parseChargeControllerInfo(dataView) {
    ...
    data['battery_percentage'] = dataView.getInt16(5);
